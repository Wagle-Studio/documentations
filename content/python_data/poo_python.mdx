...
Le décorateur `@dataclass`, introduit en Python 3.7, génère automatiquement `__init__`, `__repr__` et `__eq__` à partir des attributs déclarés. C'est l'équivalent le plus direct des DTO ou entités simples que tu écris en PHP, où la déclaration des propriétés typées suffit généralement à définir la structure.
...
La prochaine leçon quitte le langage pur pour aborder l'outillage d'un projet Python data : environnements virtuels, gestion des dépendances, structure de projet, et outils de qualité de code, avec des parallèles directs vers Composer et PHPStan déjà connus.
