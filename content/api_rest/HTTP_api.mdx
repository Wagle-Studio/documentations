...
Rappelez-vous : le principe fondamental d'une API REST et de rendre accessible les ressources d'une application via des URLs et manipulées avec les méthodes HTTP.

Les ressources sont identifiées via les URI comme le rappel l'un des principes de la contrainte d'interface uniforme, par exemple :
...
- `/articles/63/comments/12`

Cette information seule n'est pas suffisante pour informer le serveur de vos attentions selon que vous souhaitiez consulter, modifier ou supprimer une ressource.

Ce sont les méthodes HTTP qui permettent d'exprimer l'action que vous souhaitez réaliser.

| **Action (BREAD)** | **Méthode HTTP** | **Exemple d'URL** | **Signification**                           |
| ------------------ | ---------------- | ----------------- | ------------------------------------------- |
...
Une requête HTTP est composée de trois parties : la ligne de requête, les en-têtes (headers) et le corps (body).
