...
Cette leçon couvre l'outillage autour du langage : comment isoler les dépendances d'un projet Python, comment les gérer, comment structurer un projet data classique, et quels outils vérifient la qualité du code. C'est un passage obligé avant d'écrire du code Python en conditions réelles, et un point où les réflexes PHP ne se transposent pas directement.
...
La prochaine leçon revient au langage pour aborder la lecture et l'écriture de fichiers : CSV, JSON, Parquet, et la gestion des chemins avec `pathlib`, avant que le cours n'entre dans les librairies scientifiques proprement dites (NumPy, puis pandas).
