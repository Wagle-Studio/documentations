...
Pour créer une instance d'une classe, on utilise le mot‑clé `new`. Un objet est alors créé et, si défini, le [constructeur](/documentations/php_poo/constructeur_descructeur) de la classe est exécuté.
...
Une propriété est définie en utilisant au moins un [modificateur d'accès](/documentations/php_poo/visibilite). Les modificateurs disponibles sont :
...
Il est aussi possible pour les [interfaces](/documentations/php_poo/interfaces) d'avoir des constantes.
...
Le fait de déclarer une classe, une propriété ou une méthode comme statique permet de regrouper des fonctionnalités utilitaires ou des constantes sans avoir besoin de créer une instance de la classe.
