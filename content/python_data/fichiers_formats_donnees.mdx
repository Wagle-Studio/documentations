...
Pour un usage data au-delà d'un simple script, pandas (voir la leçon [Pandas : manipulation de données](/documentations/python_data/pandas_manipulation_donnees)) simplifie nettement la lecture de fichiers tabulaires, avec une seule ligne pour charger un CSV complet dans une structure exploitable directement pour l'analyse.
...
Le format Parquet est présenté en détail dans la leçon [Formats de fichiers et sérialisation](/documentations/data_engineer/formats_fichiers_serialisation) du cours data engineer : format binaire orienté colonne, avec schéma embarqué, pensé pour la lecture sélective de colonnes et une forte compression. Ce qui suit se limite à son usage concret en Python.
