...
Une [classe abstraite](/documentations/php_poo/heritage) est comme une classe partiellement construite, un document avec des espaces à remplir. Lorsqu'une classe dit "j'étends la classe abstraite Y", elle utilise des méthodes ou propriétés déjà définies dans cette autre classe.
...
À partir de PHP 8.4, les interfaces peuvent également déclarer des propriétés. Si c'est le cas, la déclaration doit spécifier si la propriété est lisible, modifiable ou les deux.
