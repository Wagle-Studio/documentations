...
Ce principe est très lié aux deux piliers de la POO que sont l'abstraction et le polymorphisme. L'abstraction permet l'exploitation de différentes classes par l'intermédiaire d'une interface unique tandis que le polymorphisme permet au code d'adopter différement comportement selon la classe exploitée.
...
Le méthode `resetPassword` de la classe `AuthController` nécessitera une modification si l'on souhaite ajouter une nouvelle manière de distribuer le lien de changement de mot de passe.
...
La méthode `send` de la classe `SmsSupplier` a était modifiée pour implémenter la même exception que la méthode de la classe `EmailSupplier`.
...
Ce principe prône l'utilisation de petites interfaces spécifique plutôt que de grandes interfaces générales. 
