...
🔗 [Exemples de VirtualHost - Apache officiel](https://httpd.apache.org/docs/2.2/fr/vhosts/examples.html)
...
Les dossiers `/etc/apache2/sites-available` et `/etc/apache2/sites-enabled` ont un rôle important à connaitre.
...
```bash
sudo a2dissite 000-default.conf # désactive 000-defaut.conf (HTTP port 80)
...
Pour héberger un site avec Apache, il faut créer un virtual host, c’est-à-dire un fichier de configuration dédié à ce site.
...
- `ServerName` : Indique le domaine principal, ça peut être un nom de domaine ou une adresse IP.

- `ServerAlias` : Variantes du domaine principal.

- `DocumentRoot` : Spécifie le dossier racine du site.

- `Directory` : Définit les règles d’accès pour le dossier spécifié. Dans cet exemple :

    - `Options` `Indexes` `FollowSymLinks` : autorise la navigation dans le répertoire et le suivi des liens symboliques.
...
- `Log files` : Définissent où enregistrer les erreurs et les logs d’accès pour ce site.

## Activer un virtual host

---

Une fois le fichier de configuration créé, il faut l’activer pour qu’Apache le prenne en compte.

Activez le virtual host, puis redémarrez le service Apache :
