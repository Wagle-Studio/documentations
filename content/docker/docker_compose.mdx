...
Docker-compose est un outil permettant de définir et gérer plusieurs conteneurs à partir d’un fichier unique : `docker-compose.yml`. Plutôt que de lancer manuellement chaque service, tout est automatisé et orchestré via un unique fichier.
...
Cette configuration ne sert pas à construire directement l’image personnalisée, car cette tâche est gérée par le [Dockerfile](/documentations/docker/docker_file).
...
services:
  ...
  php:
    ...
    volumes:
      - ./php:/app # dossier local monté dans le conteneur
    ports:
      - 8000:8000
```
